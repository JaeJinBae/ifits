...
<script type="text/javascript">
	$(function(){
        ...
        $("#searchBtn").click(function(){
    		...
    		location.href="adminInformation${pageMaker.makeQuery(1)}&searchType="+searchType+"&keyword="+keyword;
    	});
...
<body>
	...
	<div class="contentWrap">
		...
		<div class="centerMenu"> 
			<h1 class="boardTitle">&lt;Information 관리&gt;</h1>
			<div class="tbl_board">
				<table>
					<tr class="tbl_header">
						<th>번호</th>
					...
					<c:choose>
					    ...
					    <c:otherwise>
					        <c:forEach var="item" items="${list}">
								<tr>
									<td>${item.bno}</td>
									<td class="title"><a href="adminInformationRead${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}">${item.title}</a></td>
									<td><fmt:formatDate type="date" value="${item.regdate}"/></td>
									<td>${item.cnt}</td>
									<td>
										<a href="${pageContext.request.contextPath}/admin/adminInformationUpdate${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>수정</button></a>
										<a href="${pageContext.request.contextPath}/admin/adminInformationDelete${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>삭제</button></a>
									</td>
								</tr>	
							</c:forEach>
					    </c:otherwise> 
				...
				<p class="registerBtn"><a href="adminInformationRegister${pageMaker.makeSearch(pageMaker.cri.page)}"><button>글쓰기</button></a></p>
				<div class="page">
					<ul>
