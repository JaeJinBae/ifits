...
<style>
	...
	*{ 
	...
	.contentWrap .leftMenu ul li:nth-child(5) a{
		font-weight:600;
	}
...
<body>
	...
	<div class="contentWrap">
		...
		<div class="centerMenu">
			<h1 class="boardTitle">&lt;Review 관리&gt;</h1>
			<div class="notice_content">
				...
				<hr>
				<p class="btnWrap">
					<a href="${pageContext.request.contextPath}/admin/adminReview${pageMaker.makeSearch(pageMaker.cri.page)}"><button>목록</button></a>
					<a href="${pageContext.request.contextPath}/admin/adminReviewUpdate${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>수정</button></a>
					<a href="${pageContext.request.contextPath}/admin/adminReviewDelete${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>삭제</button></a>
				</p>
			</div><!-- notice_content end -->
			<c:if test="${item.state=='대기'}">
				<form id="form1" method="post" action="adminReviewAddReply">
					<div id="container">
						<h2>답변</h2>
						...
						<div class="btn">
							...
							<a href="${pageContext.request.contextPath}/admin/adminReview"><button type="button">뒤로가기</button></a>
						</div>
					</div>
				</form>
			</c:if>
			<c:if test="${item.state=='완료'}">
				<div class="notice_content">
					...
					<p class="btnWrap">
						<a href="${pageContext.request.contextPath}/admin/adminReview${pageMaker.makeSearch(pageMaker.cri.page)}"><button>목록</button></a>
						<a href="${pageContext.request.contextPath}/admin/adminReviewReplyUpdate${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${reply.bno}"><button>수정</button></a>
						<a href="${pageContext.request.contextPath}/admin/adminReviewReplyDelete${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${reply.bno}"><button>삭제</button></a>
					</p>
				</div><!-- notice_content end -->
			</c:if>
		</div><!-- centerMenu end -->
